...
flash:	all
	$(AVRDUDE) -U flash:w:ethernet.hex:i
...
load: all
	bootloadHID ethernet.hex

clean:
	rm -f ethernet.hex $(BUILDDIR)/*.o $(BUILDDIR)/*.d $(BUILDDIR)/*.elf

# file targets:
$(BUILDDIR)/main.elf: $(OBJECTS)
	$(COMPILE) -o $(BUILDDIR)/main.elf $(OBJECTS) -lm -Wl,--gc-sections

ethernet.hex: $(BUILDDIR)/main.elf
	rm -f ethernet.hex
	avr-objcopy -j .text -j .data -O ihex $(BUILDDIR)/main.elf ethernet.hex
